    ...
    icon: (
      ...
      >
        <path
          ...
          strokeLinejoin="round"
          strokeWidth={2}
          d="M3 7h10a4 4 0 110 8H9m0 0l3-3m-3 3l3 3"
        />
      </svg>
    ),
  },
...
export default function AdminDashboard({
  ...
  const returnLogs = orderLogs.filter(
    (log) => String(log.action || "").toLowerCase() === "return",
  );
              ...
              {item.isParent ? (
                <div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`transition-colors ${expandedSections.includes(item.label) ? "text-violet-400" : "text-slate-500 group-hover:text-slate-300"}`}
                      >
                        {item.icon}
                      </span>
                      {item.label}
                  ...
                  {expandedSections.includes(item.label) && item.children && (
                    <div className="ml-3 mt-0.5 pl-3 border-l border-slate-700 space-y-0.5">
                      {item.children.map((child) => (
                        ...
                        >
                          <span
                            className={`transition-colors ${currentSection === child.label ? "text-violet-200" : "text-slate-500 group-hover:text-slate-300"}`}
                          >
                            {child.icon}
                          </span>
                          {child.label}
                ...
                >
                  <span
                    className={`transition-colors ${currentSection === item.label ? "text-violet-200" : "text-slate-500 group-hover:text-slate-300"}`}
                  >
                    {item.icon}
                  </span>
                  {item.label}
        ...
        {currentSection === "Product Management" ? (
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              Track all order activities with representative details
            </p>
            {orderLogsLoading ? (
              <div className="text-gray-400 text-center py-8">
                Loading logs...
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              Returns recorded by sales representatives
            </p>
            {orderLogsLoading ? (
              <div className="text-gray-400 text-center py-8">
                Loading returns...
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-gray-600 mb-4 text-sm">
              Track all payment activities with representative details
            </p>
            {paymentLogsLoading ? (
              <div className="text-gray-400 text-center py-8">
                Loading logs...
