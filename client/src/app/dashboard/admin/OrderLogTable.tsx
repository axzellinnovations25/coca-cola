...
function renderOrderDetails(details: any) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <span className="font-semibold text-gray-600">Shop ID:</span>
          <span className="ml-2 text-gray-800">{details.shop_id || '-'}</span>
        </div>
        <div>
          <span className="font-semibold text-gray-600">Total:</span>
          <span className="ml-2 text-gray-800">{details.total ? `${Number(details.total).toFixed(2)} LKR` : '-'}</span>
        </div>
        <div>
        ...
        <div>
          <span className="font-semibold text-gray-600">Items Count:</span>
          <span className="ml-2 text-gray-800">{details.items ? details.items.length : 0}</span>
        </div>
      </div>
      
      {details.items && details.items.length > 0 && (
        <div>
          <div className="font-semibold text-sm text-gray-600 mb-2">Order Items:</div>
            ...
            {details.items.map((item: any, index: number) => (
              <div key={index} className="flex justify-between items-center py-1 border-b last:border-0">
                ...
                <span className="text-sm text-gray-600">
                  Qty: {item.quantity} × {Number(item.unit_price).toFixed(2)} LKR = {Number(item.total).toFixed(2)} LKR
                </span>
              </div>
            ))}
          </div>
...
function renderReturnDetails(details: any) {
  if (!details) return <div className="text-gray-400">No details available</div>;
  const returnedItems = Array.isArray(details.returned_items) ? details.returned_items : [];
  const previousTotal = details.previous_total ?? null;
  const newTotal = details.new_total ?? null;
  const totalReduced =
    previousTotal !== null && newTotal !== null
      ? Number(previousTotal) - Number(newTotal)
      : null;
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <span className="font-semibold text-gray-600">Previous Total:</span>
          <span className="ml-2 text-gray-800">
            {previousTotal !== null ? `${Number(previousTotal).toFixed(2)} LKR` : '-'}
          </span>
        </div>
        <div>
          <span className="font-semibold text-gray-600">New Total:</span>
          <span className="ml-2 text-gray-800">
            {newTotal !== null ? `${Number(newTotal).toFixed(2)} LKR` : '-'}
          </span>
        </div>
        <div>
          <span className="font-semibold text-gray-600">Total Reduced:</span>
          <span className="ml-2 text-gray-800 font-semibold">
            {totalReduced !== null ? `${Number(totalReduced).toFixed(2)} LKR` : '-'}
          </span>
        </div>
        <div>
          <span className="font-semibold text-gray-600">Returned Items:</span>
          <span className="ml-2 text-gray-800">{returnedItems.length}</span>
        </div>
      </div>

      {returnedItems.length > 0 && (
        <div>
          <div className="font-semibold text-sm text-gray-600 mb-2">Returned Items:</div>
          <div className="bg-gray-50 rounded-lg p-3">
...
export default function OrderLogTable({ logs }: { logs: Log[] }) {
  ...
  function exportCSV() {
    const rows = [
      ['Date', 'Shop', 'Sales Representative', 'Role', 'Action', 'Order Total', 'Details'],
      ...sortedLogs.map(l => [
        l.created_at,
        l.shop_name || '-',
    ...
    a.href = url;
    a.download = new Date().toISOString().split('T')[0] + '_order_logs.csv';
  ...
  return (
    <div className="overflow-x-auto">
      <div className="flex flex-col sm:flex-row gap-2 mb-4 items-center">
        <input
          type="text"
          placeholder="Search by shop, representative, action, or details..."
                ...
                >
                  ...
                  <td className="px-4 py-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                      ...
                      log.action === 'return' ? 'bg-orange-100 text-orange-700' :
                  ...
                  <td className="px-4 py-2">
                    ...
                    <button
                      className="ml-2 px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 text-xs text-gray-700 border border-gray-200 transition-colors"
                ...
                {isExpanded && (
                  <tr className="bg-gray-50">
                    <td colSpan={7} className="px-4 py-3">
                      {log.action === 'create' ? renderOrderDetails(log.details) :
                        log.action === 'return' ? renderReturnDetails(log.details) : (
                          <pre className="whitespace-pre-wrap break-all text-xs text-gray-900 bg-gray-100 rounded p-3 border border-gray-200">
