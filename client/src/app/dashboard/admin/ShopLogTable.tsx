...
function renderEditDiff(before: any, after: any) {
  const fields = ['name', 'address', 'owner_nic', 'email', 'phone', 'sales_rep_id', 'max_bill_amount', 'max_active_bills'];
  return (
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        {fields.map(field => (
          <div key={field} className={before[field] !== after[field] ? 'text-red-600 font-semibold' : ''}>
            <span className="font-medium text-gray-600">{field}:</span> <span className="text-gray-700">{before[field] ?? '-'}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        {fields.map(field => (
          <div key={field} className={before[field] !== after[field] ? 'text-green-600 font-semibold' : ''}>
            <span className="font-medium text-gray-600">{field}:</span> <span className="text-gray-700">{after[field] ?? '-'}</span>
          </div>
        ))}
      </div>
    </div>
...
const sortableCols = ['created_at', 'shop_name', 'user_email', 'user_role', 'action'] as const;
type SortCol = typeof sortableCols[number];

export default function ShopLogTable({ logs }: { logs: Log[] }) {
  ...
  if (!logs || logs.length === 0) {
    return <div className="text-gray-400 text-center py-8">No shop logs found.</div>;
  }
  ...
  function exportCSV() {
    const rows = [
      ['Date', 'Shop', 'User', 'Role', 'Action', 'Details'],
      ...sortedLogs.map(l => [
        l.created_at,
        l.shop_name,
        l.user_email,
  ...
  return (
    <div className="overflow-x-auto">
      <div className="flex flex-col sm:flex-row gap-2 mb-4 items-center">
        <input
          type="text"
          placeholder="Search by shop, user, action, or details..."
      ...
      <table className="min-w-full text-sm text-left">
        <thead>
          <tr className="bg-gray-50">
            ...
            <th className="px-4 py-2 font-semibold text-gray-600 cursor-pointer" onClick={() => handleSort('shop_name')}>
          ...
          {paginatedLogs.map(log => {
            ...
            return (
              <React.Fragment key={log.id}>
                ...
                >
                  <td className="px-4 py-2 text-xs text-gray-400">{log.created_at ? new Date(log.created_at).toLocaleString() : '-'}</td>
                  <td className="px-4 py-2">{log.shop_name || '-'}</td>
